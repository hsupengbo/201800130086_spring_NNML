...
class RPN_REGR_Loss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        try:
            cls = target[0, :, 0]
            regrow = target[0, :, 1:3]
            regrow_keep = (cls == 1).nonzero()[:, 0]
            regrow_true = regrow[regrow_keep]
            regrow_pred = input[0][regrow_keep]
            diff = torch.abs(regrow_true - regrow_pred)
...
class CTPN_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        regrow = self.rpn_regress(x)

        cls = cls.permute(0, 2, 3, 1).contiguous()
        regrow = regrow.permute(0, 2, 3, 1).contiguous()

        cls = cls.view(cls.size(0), cls.size(1) * cls.size(2) * 10, 2)
        regrow = regrow.view(regrow.size(0), regrow.size(1) * regrow.size(2) * 10, 2)
